<!-- TLP:CLEAR -->

# Support

Thank you for using this project! Here's how to get help.

## Documentation

Before seeking support, please check our comprehensive documentation:

- **[README.md](./README.md)** - Project overview and quick start
- **[CONTRIBUTING.md](./CONTRIBUTING.md)** - Development setup and guidelines
- **[/docs](./docs/)** - Comprehensive project documentation
  - [Quick Start Guide](./docs/QUICK_START.md)
  - [Architecture Documentation](./docs/architecture/)
  - [Component Documentation](./docs/components/)
  - [API Documentation](./docs/api/)
  - [Security Documentation](./docs/security/)

## Getting Help

### Questions and Discussions

For general questions, usage help, or discussions:

1. **GitHub Discussions** (Recommended)
   - Go to [Discussions](https://github.com/dcyfr-labs/dcyfr-labs/discussions)
   - Search existing discussions first
   - Create a new discussion if your question hasn't been answered

2. **Contact Form**
   - Visit [www.dcyfr.ai/contact](https://www.dcyfr.ai/contact)
   - Use for general inquiries or feedback

### Bug Reports

If you've found a bug:

1. **Search existing issues** first: [GitHub Issues](https://github.com/dcyfr-labs/dcyfr-labs/issues)
2. If not found, **create a new issue** using the [Bug Report template](./.github/ISSUE_TEMPLATE/bug_report.md)
3. Include:
   - Clear description of the bug
   - Steps to reproduce
   - Expected vs actual behavior
   - Environment details (OS, browser, version)
   - Screenshots or console errors (if applicable)

### Feature Requests

To suggest a new feature:

1. **Check existing feature requests**: [GitHub Issues](https://github.com/dcyfr-labs/dcyfr-labs/issues?q=is%3Aissue+label%3Aenhancement)
2. If not found, **create a new issue** using the [Feature Request template](./.github/ISSUE_TEMPLATE/feature_request.md)
3. Clearly describe:
   - The problem you're trying to solve
   - Your proposed solution
   - Use cases and benefits

### Security Vulnerabilities

**Do NOT create public issues for security vulnerabilities.**

Instead, please report security issues responsibly:

1. **GitHub Security Advisories** (Preferred)
   - Go to [Security Advisories](https://github.com/dcyfr-labs/dcyfr-labs/security/advisories)
   - Click "Report a vulnerability"
   - Provide detailed information

2. **Direct Contact**
   - Use the [contact form](https://www.dcyfr.ai/contact)
   - Include "SECURITY" in the subject line

See [SECURITY.md](./SECURITY.md) for full security policy.

## Response Times

While this is a personal project, we aim to respond to:

- **Security vulnerabilities**: Within 48 hours
- **Bug reports**: Within 3-5 business days
- **Feature requests**: Within 7 business days
- **General questions**: As time permits

Response times are estimates and may vary based on complexity and availability.

## Contributing

Interested in contributing? Great!

1. Read [CONTRIBUTING.md](./CONTRIBUTING.md) for development setup
2. Check [CODE_OF_CONDUCT.md](../governance/CODE_OF_CONDUCT.md) for community guidelines
3. Review [docs/operations/todo.md](./docs/operations/todo.md) for current priorities
4. See [open issues](https://github.com/dcyfr-labs/dcyfr-labs/issues) for contribution opportunities

## Project Resources

- **Website**: [www.dcyfr.ai](https://www.dcyfr.ai)
- **Repository**: [github.com/dcyfr-labs/dcyfr-labs](https://github.com/dcyfr-labs/dcyfr-labs)
- **Issues**: [GitHub Issues](https://github.com/dcyfr-labs/dcyfr-labs/issues)
- **Discussions**: [GitHub Discussions](https://github.com/dcyfr-labs/dcyfr-labs/discussions)
- **Security**: [Security Policy](./SECURITY.md)

## Community Guidelines

Please follow our [Code of Conduct](../governance/CODE_OF_CONDUCT.md) in all interactions. We're committed to providing a welcoming and inclusive environment for everyone.

## Additional Help

For questions not covered above:

- **Blog**: Check [www.dcyfr.ai/blog](https://www.dcyfr.ai/blog) for articles and tutorials
- **Contact**: Use the [contact form](https://www.dcyfr.ai/contact) for direct communication

---

Thank you for being part of this project!
