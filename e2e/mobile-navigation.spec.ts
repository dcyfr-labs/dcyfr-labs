...
test.describe('Mobile Navigation', () => {
  test('bottom navigation tab bar is visible and properly sized on iPhone 15 Pro Max', async ({
    page,
    browserName,
  }) => {
  ...
  test('bottom navigation tabs have proper touch targets (44x44pt minimum)', async ({
    page,
    browserName,
  }) => {
    test.skip(!['webkit', 'chromium'].includes(browserName), 'Mobile browsers only');
  ...
  test('active tab is visually indicated with aria-selected or active class', async ({
    page,
    browserName,
  }) => {
    test.skip(!['webkit', 'chromium'].includes(browserName), 'Mobile browsers only');
  ...
  test('mobile navigation has proper aria-labels for accessibility', async ({
    page,
    browserName,
  }) => {
    test.skip(!['webkit', 'chromium'].includes(browserName), 'Mobile browsers only');
  ...
  test('mobile sheet navigation opens with proper focus management', async ({
    page,
    browserName,
  }) => {
    test.skip(!['webkit', 'chromium'].includes(browserName), 'Mobile browsers only');
