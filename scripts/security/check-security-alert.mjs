 ...
 *   node scripts/check-security-alert.mjs https://github.com/dcyfr-labs/dcyfr-labs/security/code-scanning/2
 */
...
function parseInput(input) {
  ...
  const urlMatch = input.match(/github\.com\/([^\/]+)\/([^\/]+)\/security\/code-scanning\/(\d+)/);
  if (urlMatch) {
    return {
      owner: urlMatch[1],
      repo: urlMatch[2],
  ...
  throw new Error('Invalid input. Provide an alert number or full GitHub security alert URL.');
}
...
function checkAlert(owner, repo, alertNumber) {
  try {
    ...
    if (alert.most_recent_instance) {
      ...
      console.log(`   Location: ${loc.path}:${loc.start_line}-${loc.end_line}`);
    }
    ...
    if (alert.state === 'open') {
      ...
      console.log('🔕 Alert has been DISMISSED (reason: ' + alert.dismissed_reason + ')');
      return 0;
    } else {
