...
describe('FloatingFilterFab', () => {
  ...
  it('renders FAB button', () => {
    render(<FloatingFilterFab activeFilterCount={0} hasFilters={false} />);
    const button = screen.getByRole('button');
    expect(button).toBeInTheDocument();
  });

  it('shows filter count badge when filters are active', () => {
    render(<FloatingFilterFab activeFilterCount={3} hasFilters={true} />);
  ...
  it('opens filter sheet on click', () => {
    render(<FloatingFilterFab activeFilterCount={1} hasFilters={true} />);
  ...
  it('updates aria-label based on filter status', () => {
    const { rerender } = render(<FloatingFilterFab activeFilterCount={0} hasFilters={false} />);

    let button = screen.getByRole('button');
    expect(button).toHaveAttribute('aria-label', 'Open filters');

    rerender(<FloatingFilterFab activeFilterCount={2} hasFilters={true} />);
