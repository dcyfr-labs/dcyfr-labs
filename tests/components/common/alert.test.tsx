...
describe('Alert', () => {
  it('renders with critical type', () => {
    const { container } = render(<Alert type="critical">Critical alert message</Alert>);
    ...
    expect(
      alertDiv || screen.getByText('Critical alert message').closest('[class*="bg"]')
    ).toBeInTheDocument();
  });

  it('renders with warning type', () => {
    const { container } = render(<Alert type="warning">Warning alert message</Alert>);

    expect(screen.getByText('Warning alert message')).toBeInTheDocument();

    const alertDiv = container.querySelector("[class*='bg-amber-500']");
    expect(
      alertDiv || screen.getByText('Warning alert message').closest('[class*="bg"]')
    ).toBeInTheDocument();
  });

  it('renders with info type', () => {
    const { container } = render(<Alert type="info">Info alert message</Alert>);

    expect(screen.getByText('Info alert message')).toBeInTheDocument();
    const alertDiv = container.querySelector("[class*='bg-primary']");
    expect(
      alertDiv || screen.getByText('Info alert message').closest('[class*="bg"]')
    ).toBeInTheDocument();
  });

  it('renders with success type', () => {
    const { container } = render(<Alert type="success">Success alert message</Alert>);

    expect(screen.getByText('Success alert message')).toBeInTheDocument();

    const alertDiv = container.querySelector("[class*='bg-green-500']");
    expect(
      alertDiv || screen.getByText('Success alert message').closest('[class*="bg"]')
    ).toBeInTheDocument();
  });

  it('defaults to info type', () => {
    const { container } = render(<Alert>Default alert message</Alert>);

    expect(screen.getByText('Default alert message')).toBeInTheDocument();
    const alertDiv = container.querySelector("[class*='bg-primary']");
    expect(
      alertDiv || screen.getByText('Default alert message').closest('[class*="bg"]')
    ).toBeInTheDocument();
  });

  it('renders with custom className', () => {
    const { container } = render(<Alert className="custom-class">Custom alert message</Alert>);
  ...
  it('displays icon for each type', () => {
    const { container } = render(<Alert type="critical">Alert</Alert>);
