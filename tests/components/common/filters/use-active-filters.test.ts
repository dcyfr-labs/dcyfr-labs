...
describe('useActiveFilters', () => {
  ...
  it('should ignore null and undefined values', () => {
    ...
    expect(result.current.activeFilters).toEqual([{ key: 'sort', value: 'newest' }]);
  });
  ...
  it('should ignore empty arrays', () => {
    ...
    expect(result.current.hasActive).toBe(true);
    expect(result.current.count).toBe(1);
    expect(result.current.activeFilters).toEqual([{ key: 'tech', value: ['react'] }]);
  });
  ...
  it('should respect default values', () => {
    ...
    expect(result.current.hasActive).toBe(true);
    expect(result.current.count).toBe(1);
    expect(result.current.activeFilters).toEqual([{ key: 'status', value: 'active' }]);
  });
