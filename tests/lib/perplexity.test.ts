...
describe('perplexity.ts', () => {
  ...
  describe('research - configuration', () => {
    it('should throw error when Perplexity is not configured', async () => {
      ...
      const messages: ChatMessage[] = [{ role: 'user', content: 'Test query' }];

      await expect(research(messages)).rejects.toThrow('Perplexity API key not configured');
    });
  ...
  describe('research - successful requests', () => {
    ...
    it('should include citations when requested', async () => {
      const mockResponse: PerplexityResponse = {
        ...
        citations: ['https://example.com/1', 'https://example.com/2', 'https://example.com/3'],
        choices: [
          {
            index: 0,
            finish_reason: 'stop',
      ...
      const messages: ChatMessage[] = [{ role: 'user', content: 'Research query' }];
    ...
    it('should include related questions when requested', async () => {
      const mockResponse: PerplexityResponse = {
        ...
        related_questions: ['What is React Server Components?', 'How do React hooks work?'],
        choices: [
          {
            index: 0,
            finish_reason: 'stop',
      ...
      const messages: ChatMessage[] = [{ role: 'user', content: 'Tell me about React' }];
    ...
    it('should use custom model when specified', async () => {
      ...
      const messages: ChatMessage[] = [{ role: 'user', content: 'Quick query' }];
    ...
    it('should apply search recency filter', async () => {
      ...
      const messages: ChatMessage[] = [{ role: 'user', content: 'Latest news' }];
  ...
  describe('research - caching', () => {
    ...
    it('should cache successful results', async () => {
      ...
      const messages: ChatMessage[] = [{ role: 'user', content: 'Cache test' }];
    ...
    it('should not cache when useCache is false', async () => {
      ...
      const messages: ChatMessage[] = [{ role: 'user', content: 'No cache test' }];
    ...
    it('should clear cache', async () => {
      ...
      const messages: ChatMessage[] = [{ role: 'user', content: 'Clear test' }];
