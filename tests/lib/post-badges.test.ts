...
describe('getPostBadgeMetadata', () => {
  ...
  describe('Latest Post Badge', () => {
    it('identifies the most recent post as latest', async () => {
      vi.mocked(viewsModule.getMultiplePostViews).mockResolvedValue(
        new Map([
          ['post-1', 100],
          ['post-2', 200],
          ['post-3', 50],
  ...
  describe('Integration with Views Service', () => {
    it('calls getMultiplePostViews with eligible post IDs', async () => {
      ...
      expect(viewsModule.getMultiplePostViews).toHaveBeenCalledWith(['post-1', 'post-2', 'post-3']);
  ...
  describe('Type Safety', () => {
    it('returns PostBadgeMetadata with correct shape', async () => {
      vi.mocked(viewsModule.getMultiplePostViews).mockResolvedValue(new Map([['post-1', 100]]));
    ...
    it('returns string slugs or null values', async () => {
      vi.mocked(viewsModule.getMultiplePostViews).mockResolvedValue(new Map([['post-2', 100]]));
